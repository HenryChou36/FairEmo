...
#%% testing
    ...
    RE_BOX['test']['metric'][0, 3] = mean_squared_error(pred, true, multioutput='raw_values')
    RE_BOX['test']['metric'][0, 4] = mean_absolute_percentage_error(pred, true, multioutput='raw_values')
    RE_BOX['test']['metric'][0, 5] = utils.metric.mean_parity_diff(pred, gender, MIN_SAMPLE)
    RE_BOX['test']['metric'][0, 6] = utils.metric.mean_parity_diff(pred, spk_id, MIN_SAMPLE)
    RE_BOX['test']['metric'][0, 7] = utils.metric.mean_parity_diff(pred, clu_id, MIN_SAMPLE)
    joblib.dump(RE_BOX, args.exp_name+'/info_ckpt.pkl')
